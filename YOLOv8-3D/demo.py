...
def process2D(image, track=True, device='cpu'):
    ...
    if track is True:
        ...
        for id_ in list(tracking_trajectories.keys()):
            if id_ not in [int(bbox.id) for predictions in results if predictions is not None for bbox in
                           predictions.boxes if bbox.id is not None]:
        ...
        for predictions in results:
            ...
            for bbox, masks in zip(predictions.boxes, predictions.masks):
                ...
                for scores, classes, bbox_coords, id_ in zip(bbox.conf, bbox.cls, bbox.xyxy, bbox.id):
                    ...
                    label = (' ' + f'ID: {int(id_)}' + ' ' + str(predictions.names[int(classes)]) + ' ' + str(
                        round(float(scores) * 100, 1)) + '%')
                    text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 2, 1)
                    dim, baseline = text_size[0], text_size[1]
                    cv2.rectangle(image, (int(xmin), int(ymin)),
                                  ((int(xmin) + dim[0] // 3) - 20, int(ymin) - dim[1] + baseline), (30, 30, 30),
                                  cv2.FILLED)
                    cv2.putText(image, label, (int(xmin), int(ymin) - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255), 1)
        ...
        for predictions in results:
            ...
            for bbox, masks in zip(predictions.boxes, predictions.masks):
                ...
                for scores, classes, bbox_coords in zip(bbox.conf, bbox.cls, bbox.xyxy):
                    ...
                    label = (' ' + str(predictions.names[int(classes)]) + ' ' + str(
                        round(float(scores) * 100, 1)) + '%')
                    text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 2, 1)
                    dim, baseline = text_size[0], text_size[1]
                    cv2.rectangle(image, (int(xmin), int(ymin)),
            ...
            for bbox_, dim, alpha, theta_ray, orient, conf, classes, location, objID in bboxes3D:
                ...
                if BEV_plot:
                    ...
                    plot3dbev.plot(img=img3, class_object=obj.name.lower(),
                                   ...
                                   dim=[obj.h, obj.w, obj.l], loc=[obj.tx, obj.ty, obj.tz], rot_y=rot_y,
